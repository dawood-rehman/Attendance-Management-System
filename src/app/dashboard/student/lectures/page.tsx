  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div>
        ...
        <p className="text-white/40 text-sm mt-0.5">Your class lecture schedule</p>
      </div>
      ...
      <Card className="p-0 overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={<BookOpen size={32} />} title="No lectures found" description="Lectures will appear here after admin schedules them for your class" />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  {['Lecture', 'Teacher', 'Schedule', 'Room', 'Status'].map((header) => (
              ...
              <tbody>
                {lectures.map((lecture) => (
                  <tr key={lecture._id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                    ...
                    <td className="px-5 py-3.5 text-white/50">{refName(lecture.teacherId)}</td>
